...
const Contact = () => {
        ...
        return ( <
                ...
                a href = "mailto:kasongokakumbiguy@gmail.com"
                className = 'bg-blue-900 px-2 rounded-lg border border-solid shadow-xl hover:text-yellow-600' > Send a message < /a> <
                /article>
                ...
                a href = { `https://wa.me/${aboutData?.phoneNumber}` }
                className = 'bg-blue-900 px-2 rounded-lg border border-solid shadow-xl hover:text-red-600' > Send a message < /a> <
                /article > <
                /div> <
                form onSubmit = { handleSendEmail }
                className = 'flex flex-col space-y-4 w-96 justify-start' >
                <
                input type = "text"
                name = 'name'
                value = { name }
                ...
                required className = 'p-1 rounded-sm bg-transparent block border border-solid border-blue-800' / >
                <
                input type = "email"
                value = { sender }
                onChange = {
                ...
                rows = "7"
                className = 'p-1 rounded-sm bg-transparent block border border-solid border-blue-800' > < /textarea> <
                button type = 'submit'
                className = 'bg-green-800 hover:bg-green-900 w-28 rounded-sm text-zinc-300' > { isLoading ? "Sending... Message" : "Send Message" } < /button> {
                    isError ? <p p className = "text-red-600" > "Can't send your message, please try again</p>:"
                    "} {
