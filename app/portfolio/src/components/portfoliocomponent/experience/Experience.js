...
import { useGetExperienceQuery } from '../../../redux_tool.js/service/dataApi/apiDataService'
import ExperienceSkeleton from './ExperienceSkeleton'

const Experience=()=>{
  const {data, isError, error , isLoading}=  useGetExperienceQuery()
    ...
    <section id='experience'  className='p-2'>
      ...
      <div className=" space-y-10 md:space-y-0 md:space-x-10 items-center md:items-start px-20">
         <ul className="flex object-center justify-center items-center ">
          {data && data.map((item, index)=><li className='grid grid-cols-1 place-self-center items-center justify-center  md:grid-cols-2 lg:grid-cols-2 h-fit w-fit space-y-2 md:space-x-2'>
          ...
          <div className=" w-full h-full md:px-20">
            ...
            <ul  className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 w-full">
              {data && item.frontend.map((item,index)=><li key={index} className="w-full ">
         ...
         <h3 className='flex text-zinc-300'>Backend Development</h3> 
          ...
          <ul className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 w-fit">
            {data && item.backend.map((item, index)=><li className="w-full">
