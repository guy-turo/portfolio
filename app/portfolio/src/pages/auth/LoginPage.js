...
function LoginPage() {
  const bag2 = ""
  return (
    <div  className=' flex w-full snap-y snap-mandatory h-screen overflow-x-hidden overflow-y-hidden justify-center lg:justify-end items-center px-10 place-content-center'>
     <div  className="flex snap-center  flex-col  items-center space-y-4 bg-blue-950 justify-evenly rounded-md p-5 w-96 h-fit">
      
      <form action="" className='w-full space-y-5'>
      <h1 className="text-gray-300 block text-xl">Log in to Portfolio</h1>
  ...
  <div class="flex mt-4">
    ...
    <label for="hs-toggle-password-checkbox" class="text-sm text-gray-500 ms-3 dark:text-neutral-400">Show password</label>
      ...
      <button className='rounded-lg bg-green-600 w-full p-1 text-black font-semibold'>Login</button>
      </form>
      <p className='flex place-items-center items-center justify-center'>Don't have an account? <span>Sign Up</span></p>
     </div>
    </div>
  )
}
