...
const projectImageUpload = async(req, res, next) => {
    ...
    try {
        ...
        const newImage = new ProjectImagesModel({
            title: req.body.title,
            description: req.body.description,
            imageUrl: file.path
        })
...
//cloudinary don't accept update func but we can delete and add it
const projectUpdateImage = async(req, res) => {
    try {
        const id = req.params.Id
        const image = await ProjectImagesModel.findById(itemId)
...
const projectDeleteImage = async(req, res) => {
    try {
        const id = req.params.Id;
        const image = await ProjectImagesModel.findById(id)
        if (!image) {
            return res.status(404).json({ message: "Item not found" })
        }
        const publicId = image.imageId;
...
const projectFetchImage = async(req, res) => {

    try {
        const image = await ProjectImagesModel.findById(req.params.Id)
        if (!image) {
            return res.status(404).json({ message: 'Image not found' })
        }
        const imageUrl = cloudinary.image(item.imageId, { width: 400 })
...
const projectFetchImages = async(req, res) => {
    console.log("to fetch project")
    try {
        const response = await ProjectImagesModel.find()
        if (!response) {
            return res.status(404).json({ message: 'Image not found' })
        }
